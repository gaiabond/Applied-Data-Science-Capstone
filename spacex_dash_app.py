...
launch_sites = list(spacex_df['Launch Site'])
launch_sites = [*set(launch_sites)]

options_list = [{'label': 'All Sites', 'value': 'ALL'}]
for site in launch_sites:
    el = {'label' : site, 'value': site}
...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        data = filter_df.groupby(['class'])['class'].count()
...
def get_scatter_chart(entered_site, entered_payload):
    if entered_site == 'ALL':
        filter_df = spacex_df[(spacex_df['Payload Mass (kg)']>entered_payload[0]) & (spacex_df['Payload Mass (kg)']<entered_payload[1]) ]
        fig = px.scatter(filter_df, x = 'Payload Mass (kg)', 
        y = 'class', color="Booster Version Category",
        title = 'Correlation between Payload and Success for all the Sites')
        return fig
    else:
        filter_df = spacex_df[(spacex_df['Payload Mass (kg)']>entered_payload[0]) & (spacex_df['Payload Mass (kg)']< entered_payload[1]) ]
        filter_df = filter_df[filter_df['Launch Site'] == entered_site]
